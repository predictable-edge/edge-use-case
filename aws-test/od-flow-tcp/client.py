...
def send_requests(client_socket, num_requests, packets_per_request, interval_ms, packet_size, send_queue, start_times, sending_done, data_lock):
    ...
    for request_index in range(num_requests):
        ...
        # Add request index to send queue
        with data_lock:
...
def receive_responses(client_socket, send_queue, start_times, latency_results, sending_done, data_lock):
    ...
    while not sending_done.is_set() or send_queue:
        try:
            ...
            if response:
                # Assuming responses are received in the same order as requests
                with data_lock:
                    if send_queue:
                        request_index = send_queue.popleft()
                        # Record end time
                        end_time = time.time()
