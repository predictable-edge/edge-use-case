...
def parse_latency_file(file_path, start_line=0):
    ...
    with open(file_path, 'r') as f:
        ...
        for line in f:
            try:
                # Extract latency value (assuming format contains "X ms")
                parts = line.split()
...
def generate_cdf_plot(latencies, output_path, start_line):
    ...
    # Customize plot
    plt.title(f'Latency CDF (starting from line {start_line})', fontweight='bold')
...
def main():
    ...
    if args.output_dir:
        ...
        # Default: replace 'result' with 'figure' in the path
        output_dir = os.path.dirname(args.input_path).replace('result', 'figure')
    
    # Create filename based on input filename but always use .pdf extension
