...
std::map<int64_t, int64_t> pull_timestamps;

std::map<int64_t, int64_t> push_timestamps_after_enc;
std::map<int64_t, int64_t> pull_timestamps_before_dec;
...
void *push_stream(void *args) {
    ...
    int encode_ret = pthread_create(&encode_thread_id, NULL, encode_thread_func, &push_ctx);
    if (encode_ret != 0) {
...
void* pull_stream(void* args) {
    char** my_args = static_cast<char**>(args);
    char* input_url = my_args[0];

    pthread_mutex_lock(&cout_mutex);
    std::cout << "[Pull Thread] Starting pull_stream..." << std::endl;
    pthread_mutex_unlock(&cout_mutex);
    ...
    if (ret < 0) {
        ...
        char errbuf[AV_ERROR_MAX_STRING_SIZE];
        av_strerror(ret, errbuf, sizeof(errbuf));
        std::cerr << "[Pull Thread] Could not open input: " << errbuf << std::endl;
        pthread_mutex_unlock(&cout_mutex);
    ...
    if (ret < 0) {
        ...
        char errbuf[AV_ERROR_MAX_STRING_SIZE];
        av_strerror(ret, errbuf, sizeof(errbuf));
        std::cerr << "[Pull Thread] Failed to retrieve input stream information: " << errbuf << std::endl;
        pthread_mutex_unlock(&cout_mutex);
        avformat_close_input(&input_fmt_ctx);
        av_dict_free(&options);
        avformat_network_deinit();
    ...
    if (video_stream_index == -1) {
        pthread_mutex_lock(&cout_mutex);
        std::cerr << "[Pull Thread] No video stream found" << std::endl;
        pthread_mutex_unlock(&cout_mutex);
        avformat_close_input(&input_fmt_ctx);
        av_dict_free(&options);
        avformat_network_deinit();
    ...
    if (!codec) {
        pthread_mutex_lock(&cout_mutex);
        std::cerr << "[Pull Thread] Could not find the decoder" << std::endl;
        pthread_mutex_unlock(&cout_mutex);
        avformat_close_input(&input_fmt_ctx);
        av_dict_free(&options);
        avformat_network_deinit();
    ...
    if (!codec_ctx) {
        pthread_mutex_lock(&cout_mutex);
        std::cerr << "[Pull Thread] Could not allocate codec context" << std::endl;
        pthread_mutex_unlock(&cout_mutex);
        avformat_close_input(&input_fmt_ctx);
        av_dict_free(&options);
        avformat_network_deinit();
    ...
    if (ret < 0) {
        pthread_mutex_lock(&cout_mutex);
        std::cerr << "[Pull Thread] Failed to copy codec parameters to codec context" << std::endl;
        pthread_mutex_unlock(&cout_mutex);
    ...
    if (ret < 0) {
        pthread_mutex_lock(&cout_mutex);
        std::cerr << "[Pull Thread] Could not open codec" << std::endl;
        pthread_mutex_unlock(&cout_mutex);
        avcodec_free_context(&codec_ctx);
        avformat_close_input(&input_fmt_ctx);
        av_dict_free(&options);
    ...
    if (!frame) {
        pthread_mutex_lock(&cout_mutex);
        std::cerr << "[Pull Thread] Could not allocate frame" << std::endl;
        pthread_mutex_unlock(&cout_mutex);
        avcodec_free_context(&codec_ctx);
        avformat_close_input(&input_fmt_ctx);
        av_dict_free(&options);
    ...
    if (!packet) {
        pthread_mutex_lock(&cout_mutex);
        std::cerr << "[Pull Thread] Could not allocate packet" << std::endl;
        pthread_mutex_unlock(&cout_mutex);
        av_frame_free(&frame);
        avcodec_free_context(&codec_ctx);
        avformat_close_input(&input_fmt_ctx);
    ...
    while (av_read_frame(input_fmt_ctx, packet) >= 0) {
        if (packet->stream_index == video_stream_index) {
            ...
            auto it = push_timestamps_after_enc.find(frame_count);
            if (it != push_timestamps_after_enc.end()) {
                pull_timestamps_before_dec[frame_count] = pull_time_ms_before_dec;
            }
            ...
            if (ret < 0) {
                ...
                char errbuf[AV_ERROR_MAX_STRING_SIZE];
                av_strerror(ret, errbuf, sizeof(errbuf));
                std::cerr << "[Pull Thread] Error sending packet for decoding: " << errbuf << std::endl;
                pthread_mutex_unlock(&cout_mutex);
                av_packet_unref(packet);
                continue;
            }
            ...
            while (ret >= 0) {
                ...
                if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
                    ...
                    char errbuf[AV_ERROR_MAX_STRING_SIZE];
                    av_strerror(ret, errbuf, sizeof(errbuf));
                    std::cerr << "[Pull Thread] Error during decoding: " << errbuf << std::endl;
                    pthread_mutex_unlock(&cout_mutex);
                    break;
                }

                int64_t pull_time_ms = get_current_time_us() / 1000;
                auto it = push_timestamps.find(frame_count);   
                if (it != push_timestamps.end()) {
                    pull_timestamps[frame_count] = pull_time_ms;
                }
                frame_count++;
            }
        }
    ...
    pthread_mutex_lock(&cout_mutex);
    std::cout << "[Pull Thread] Finished pull_stream." << std::endl;
    pthread_mutex_unlock(&cout_mutex);
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <push_input_file> <push_output_url> <pull_input_url>\n", argv[0]);
        fprintf(stderr, "Example: %s snow-scene.mp4 \"srt://192.168.2.3:9000?mode=caller&streamid=live&latency=20&sndbuf=65536\" \"srt://192.168.2.2:10000?mode=listener&streamid=transcoded&latency=20&rcvbuf=65536\"\n", argv[0]);
        return 1;
    }

    char *push_input_file = argv[1];
    char *push_output_url = argv[2];
    char *pull_input_url = argv[3];

    avformat_network_init();

    pthread_t pull_thread_id;
    char **pull_args = (char **)malloc(1 * sizeof(char *));
    if (!pull_args) {
        fprintf(stderr, "Could not allocate memory for pull_args.\n");
        exit(1);
    }
    pull_args[0] = strdup(pull_input_url);
    if (!pull_args[0]) {
        fprintf(stderr, "Could not duplicate pull arguments.\n");
        exit(1);
    }

    int pull_ret = pthread_create(&pull_thread_id, NULL, pull_stream, pull_args);
    if (pull_ret != 0) {
        fprintf(stderr, "Failed to create pull thread.\n");
        exit(1);
    }
    ...
    std::ofstream ofs("frame.log");
    if (!ofs.is_open()) {
        std::cerr << "can not open frame.log\n";
        return 1;
    }

    ofs << std::left << std::setw(10) << "Frame" 
        << std::left << std::setw(20) << "E2E latency(ms)" 
        << std::left << std::setw(20) << "Trans latency(ms)" 
        << "\n";

    for (const auto& [frame_pts, push_time_ms] : push_timestamps) {
        int64_t e2e_latency = -1; 
        auto it_e2e_pull = pull_timestamps.find(frame_pts);
        if (it_e2e_pull != pull_timestamps.end()) {
            e2e_latency = it_e2e_pull->second - push_time_ms;
        }

        int64_t trans_latency = -1;
        auto it_trans_push = push_timestamps_after_enc.find(frame_pts);
        auto it_trans_pull = pull_timestamps_before_dec.find(frame_pts);
        if (it_trans_push != push_timestamps_after_enc.end() && it_trans_pull != pull_timestamps_before_dec.end()) {
            trans_latency = it_trans_pull->second - it_trans_push->second;
        }

        ofs << std::left << std::setw(10) << frame_pts + 1
            << std::left << std::setw(20) << (e2e_latency != -1 ? std::to_string(e2e_latency) + " ms" : "N/A")
            << std::left << std::setw(20) << (trans_latency != -1 ? std::to_string(trans_latency) + " ms" : "N/A")
            << "\n";
    }
