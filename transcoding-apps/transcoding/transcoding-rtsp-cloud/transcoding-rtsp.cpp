...
bool initialize_decoder(const char* input_url, DecoderInfo& decoder_info) {
    ...
    // Add options to improve cloud connectivity
    av_dict_set(&format_opts, "timeout", "5000000", 0);          // General timeout 5 seconds
    av_dict_set(&format_opts, "reconnect", "1", 0);              // Enable reconnect
    av_dict_set(&format_opts, "reconnect_streamed", "1", 0);     // Reconnect if streamed
    av_dict_set(&format_opts, "reconnect_delay_max", "5", 0);    // Maximum reconnect delay in seconds
...
bool encode_frames(const EncoderConfig& config, FrameQueue& frame_queue, AVRational input_time_base, std::atomic<bool>& encode_finished) {
    ...
    AVDictionary* format_opts = nullptr;
    // Cloud connectivity options for output stream
    av_dict_set(&format_opts, "rtsp_transport", "udp", 0);       // Use UDP for RTP transport
    av_dict_set(&format_opts, "buffer_size", "8192000", 0);      // Increased buffer size
    av_dict_set(&format_opts, "max_delay", "500000", 0);         // 500ms max delay
    av_dict_set(&format_opts, "stimeout", "5000000", 0);         // Socket timeout 5 seconds
    av_dict_set(&format_opts, "listen_timeout", "30000000", 0);  // Extended connection timeout 30 seconds
    av_dict_set(&format_opts, "timeout", "5000000", 0);          // General timeout 5 seconds
    av_dict_set(&format_opts, "reorder_queue_size", "50", 0);    // Larger reorder queue
    av_dict_set(&format_opts, "fifo_size", "1048576", 0);        // FIFO buffer 1MB
    ...
    av_dict_set(&codec_opts, "listen_timeout", "30000000", 0);   // Extended timeout for cloud 30 seconds
    av_dict_set(&codec_opts, "max_delay", "0", 0);         
    av_dict_set(&codec_opts, "reorder_queue_size", "50", 0);     // Larger reorder queue for cloud
    av_dict_set(&codec_opts, "fifo_size", "1048576", 0);         // 1MB FIFO
    av_dict_set(&codec_opts, "buffer_size", "8192000", 0);       // 8MB buffer for cloud connectivity
    ...
    av_dict_set(&codec_opts, "fflags", "nobuffer", 0);           // Reduce buffering
    av_dict_set(&codec_opts, "rtsp_flags", "prefer_tcp", 0);     // Prefer TCP for control
    av_dict_set(&codec_opts, "timeout", "5000000", 0);           // General timeout 5 seconds
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] 
                  << " rtsp://cloud_ip:9000/stream rtsp://cloud_ip:10000/stream" 
                  << std::endl;
