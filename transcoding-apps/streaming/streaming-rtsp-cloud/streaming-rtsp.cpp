...
void* pull_stream(void* args) {
    ...
    av_dict_set(&options, "timeout", "5000000", 0);          // General timeout 5 seconds
    av_dict_set(&options, "reconnect", "1", 0);              // Enable reconnect
    av_dict_set(&options, "reconnect_streamed", "1", 0);     // Reconnect if streamed
    av_dict_set(&options, "reconnect_delay_max", "5", 0);    // Maximum reconnect delay in seconds
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        ...
        fprintf(stderr, "Example: %s snow-scene.mp4 \"rtsp://192.168.2.3:9000/stream\" \"rtsp://192.168.2.3:10000/stream\"\n", argv[0]);
        return 1;
    }
