...
def handle_client_connection(client_socket, client_address, destination_ip, destination_port):
    ...
    try:
        while True:
            # Read header (12 bytes: request_id, bytes_size, multiplier)
            header_data = recv_all(client_socket, 12)
            if not header_data:
                break
                
            request_id, bytes_size, multiplier = struct.unpack('!III', header_data)
            ...
            # Calculate response size based on multiplier
            response_size = bytes_size * multiplier

            # Send response with multiplied size
            try:
                # Send header first
                response_header = struct.pack('!II', request_id, response_size)
                response_socket.sendall(response_header)
                
                # Send response data with multiplied size
                response_data = b'\0' * response_size
                response_socket.sendall(response_data)

                print(f"Sent response for request {request_id} with size multiplier {multiplier}")
            except Exception as e:
