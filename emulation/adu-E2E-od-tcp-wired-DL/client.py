...
def send_requests(server_ip, server_port, num_requests, bytes_per_request, interval_ms, send_times, lock):
    """Function to send TCP requests from ue1 namespace.
    ...
    """
    try:
        # Switch to ue1 network namespace for sending requests
        enter_netns('ue1')
    ...
    try:
        ...
        print(f"Connected to server {server_ip}:{server_port} in ue1")
    except Exception as e:
...
def receive_responses(listen_port, num_requests, send_times, lock, result_dir):
    """Function to receive TCP responses and calculate latency on host machine.
    ...
    """
    try:
        # Create and setup receiving socket on host
...
def client_main(args):
    ...
    result_dir = os.path.join('../result/adu-E2E-od-tcp-wired-DL', timestamp)
