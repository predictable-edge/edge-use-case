...
def receive_responses(listen_port, num_requests, send_times, lock, result_dir):
    """Function to receive UDP responses in ue1 namespace and calculate latency."""
    try:
        # Enter ue1 namespace for receiving responses
        enter_netns('ue1')
    ...
    try:
        # Create and bind UDP receiving socket in ue1 namespace
        recv_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        recv_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        recv_socket.bind(('', listen_port))
        print(f"Listening for UDP responses on port {listen_port} in ue1")
    except Exception as e:
...
def client_main(args):
    ...
    result_dir = os.path.join('../result/adu-E2E-od-tcp-wired-UL-udp-wireless-DL', timestamp)
