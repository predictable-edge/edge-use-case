                ...
                if tracker.add_packet(request_id, seq_num, total_packets):
                    ...
                    with self.tracker_lock:
                        if tracker.timestamp_s0 > 0 and timestamp_c1 > 0 and tracker.timestamp_c0 > 0 and tracker.t1 > 0:
                            # Use the basic formula based on initial handshake: 
                            # t2 = t1 + (timestamp_s1-timestamp_s0) - (timestamp_c1-timestamp_c0)
                            server_time_diff = timestamp_s1 - tracker.timestamp_s0
                            client_time_diff = timestamp_c1 - tracker.timestamp_c0
                            transmission_delay = tracker.t1 + (server_time_diff - client_time_diff)
                            ...
                            print(f"  - Clock offset adjustment: {(server_time_diff - client_time_diff)*1000:.2f}ms")
                        else:
                            missing = []
